$(function() {
	...
	socket.on('server -- new user', function(data) {
		$chat_window.append('<div class="msg_status"><span class="chat_username">' + data + '</span> has entered the room</div>');
	});
	socket.on('server -- new message', function(data) {
		$chat_window.append('<div class="msg"><span class="chat_username">' + data.username + '</span>: ' + data.message + '</div>');
	});
	socket.on('server -- user left', function(data) {
		$chat_window.append('<div class="msg_status"><span class="chat_username">' + data + '</span> has left the room</div>');
	});
